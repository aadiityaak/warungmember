    ...
    <div class="mx-6 pt-6">
        <header class="mb-6 space-y-0.5">
            ...
            <p class="text-sm leading-[1.4] text-[#62625b]">
                Tambahkan menu baru ke Warung Mas Mbull
            </p>
        </header>

        <div class="max-w-2xl rounded-2xl border border-[#dadad3] bg-white p-8">
            <h3 class="mb-6 text-[18px] font-semibold leading-[1.3] text-[#000000]">Form Produk Baru</h3>

            <form @submit.prevent="submit" class="space-y-6">
                <!-- Nama -->
                <div>
                    <Label class="mb-1.5 block text-sm font-semibold leading-[1.4] text-[#000000]">Nama Produk</Label>
                    <Input id="name" v-model="form.name" placeholder="Contoh: Ayam Geprek Sambal Matah" />
                ...
                <div>
                    ...
                    <div v-if="categories.length === 0" class="text-sm text-[#91918c]">Belum ada kategori. Kategori akan muncul setelah ditambahkan via seeder.</div>
                ...
                <Button type="submit" :disabled="form.processing" class="w-full">Simpan Produk</Button>
            </form>
        </div>
    </div>
</template>
