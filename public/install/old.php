...
function handleEnvironmentSetup()
{
    ...
    $appName = trim($_POST['app_name'] ?? 'warungmember');
    $appUrl = trim($_POST['app_url'] ?? 'http://localhost');
    $dbHost = trim($_POST['db_host'] ?? 'localhost');
    $dbPort = trim($_POST['db_port'] ?? '3306');
    $dbDatabase = trim($_POST['db_database'] ?? 'warungmember_db');
    $dbUsername = trim($_POST['db_username'] ?? '');
    ...
    if (file_exists($envExamplePath)) {
        ...
        $envTemplate = 'APP_NAME=warungmember
APP_ENV=production
APP_KEY=
APP_DEBUG=false
APP_TIMEZONE=UTC
...
function handleDatabaseSetup()
{
    try {
        ...
        $dbDatabase = $env['DB_DATABASE'] ?? 'warungmember_db';
...
function createBasicData($pdo)
{
    ...
    if ($result['count'] == 0) {
        ...
        $stmt = $pdo->prepare('
            ...
            VALUES (?, ?, ?, ?, NOW(), NOW())
        ');
        $stmt->execute(['Administrator', 'admin@warungmember.local', date('Y-m-d H:i:s'), $defaultPassword]);
    }
}
...
function handleFinalSetup()
{
    try {
        ...
        if ($adminEmail && $adminPassword) {
            ...
            createAdminUser('admin@warungmember.local', 'admin123');
        }
...
function showStep($step, $message = '', $error = false)
{
    ...
    <head>
        ...
        <title>Instalasi warungmember</title>
        <style>
            :root { 
...
function showEnvironmentForm()
{
    ...
    <form method="POST">
        ...
        <div class="form-group">
            ...
            <input type="text" id="db_database" name="db_database" value="<?php echo htmlspecialchars($dbDatabase); ?>" placeholder="warungmember_db" required>
        </div>
...
function showFinalForm()
{
    ...
    <?php } else { ?>
        ...
        <div class="message" style="background: hsl(142.1 76.2% 36.3% / 0.1); color: hsl(142.1 100% 17%); border-color: hsl(142.1 76.2% 36.3% / 0.3);">
            ...
            <strong>Email:</strong> admin@warungmember.local<br>
...
function showSuccess()
{
    ...
    <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>Instalasi Berhasil - warungmember</title>
        <style>
            :root { 
                --background: 0 0% 100%; --foreground: 240 10% 3.9%; --card: 0 0% 100%; --card-foreground: 240 10% 3.9%; 
                --popover: 0 0% 100%; --popover-foreground: 240 10% 3.9%; --primary: 240 5.9% 10%; --primary-foreground: 0 0% 98%;
    ...
    <body>
        <div class="container">
            ...
            <p>warungmember telah berhasil diinstal dan dikonfigurasi. Permissions telah diperbaiki otomatis.</p>
